...
export function screenshotRequest(raw,size){
 ...
 request.search=new URLSearchParams({url,screenshot:'true',meta:'false','viewport.width':String(size.width),'viewport.height':String(size.contentHeight),'viewport.deviceScaleFactor':'1'});
 return request.href;
}
export async function captureSnapshot(raw,size,signal){
 ...
 if(shot.width!==size.width||shot.height!==size.contentHeight)throw new Error('The capture did not match the requested dimensions. Try again.');
 ...
 await new Promise((resolve,reject)=>{
  ...
  preview.onload=()=>{cleanup();if(preview.naturalWidth!==size.width||preview.naturalHeight!==size.contentHeight)reject(new Error('The image did not match the requested dimensions.'));else resolve();};
...
export function snapshotDocument(capture){
 const body=capture?.image?`<img alt="Website snapshot" src="${escape(capture.image)}">`:`<main><span>DUO VIEW</span><h1>${capture?.error?'Preview unavailable':'Preparing your preview…'}</h1><p>${escape(capture?.error||'Rendering the website at this display size. This can take a few seconds.')}</p></main>`;
 return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src https:; style-src 'unsafe-inline'"><style>*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;background:#f6f8fc;font-family:Arial,sans-serif;color:#253758}img{width:100%;height:100%;display:block;object-fit:contain;object-position:top left}main{padding:12% 9%}span{font-size:14px;letter-spacing:3px;color:#8292ac}h1{font-size:30px;line-height:1.2;font-weight:500}p{font-size:18px;line-height:1.6;color:#71819b}</style></head><body>${body}</body></html>`;
}
